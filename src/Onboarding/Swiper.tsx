...
export default class OnboardingScreens extends React.Component<Props, any> {
  ...
  initState(props: Props) {
    ...
    const total: number = props.children ? props.children.length || 1 : 0;
    // Current index
  ...
  onScrollEndDrag = (e) => {
    ...
    if (offset === newOffset &&
      (index === 0 || (children != null && index === children.length - 1))) {
      this.internals.isScrolling = false;
    }
  }
  ...
  renderPagination = () => {
    ...
    const dots: Array<React.ReactElement<any>> = [];
    ...
    return (
      <View pointerEvents="box-none" style={[styles.buttonWrapper, styles.fullScreen]}>
        {lastScreen
          ...
          ? <Button text="Start Now" onPress={() => this.doneBtnHandle()} />
